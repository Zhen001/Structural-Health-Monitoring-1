function [phi,fn,zeta,Frequency,PSD,Locs,Peaks] = AFDD_function_py(Az,Fs,Nmodes,varargin)
% [phi,fn,zeta] = AFDD(Az,Fs,Nmodes,varargin) calculate the mode shapes, 
...
%% Author Info
% Etienne Cheynet - last modified 16/05/2016
%% References
...
% decomposition. In Proc. of the 18*閬竛ternational Modal Analysis Conference
% (IMAC), San Antonio, Texas.

%% Input parser for options
t=0:1/Fs:length(Az)/Fs-1/Fs;
p = inputParser();
...
p.addOptional('Ts',60) % option for duration of autocorrelation function (for estimation of damping ratio only)
p.parse(varargin{:});
...
if lvbo
    ...
    Az=filter(bn1,an1,Az');            % 对数据进行滤波，默认每列单独滤波
    Az = Az';
end
...
if dataPlot==1
    figure
%     plot(f,S);
%     xlim([0,10]);ylim([min(S(1,:)),1.1*max(S(1,:))]);
    loglog(f,S)
    ylim([min(S(1,:))*10,max(S(1,:))*10]);
    xlabel('Frequency (Hz)')
...
if isempty(fnTarget)
    if strcmpi(PickingMethod,'auto')
        ...
        if dataPlot==1
            figure
%             plot(newF,newS,'k-',newF(indMax),newS(indMax),'ko','markerfacecolor','r')
%             ylim([min(newS(1,:)),1.1*max(newS(1,:))]);xlim([0,10])
            loglog(newF,newS,'k-',newF(indMax),newS(indMax),'ko','markerfacecolor','r')
            ylim([min(newS(1,:)),max(newS(1,:))*10]);
            xlabel('Frequency (Hz)')
            ylabel('Normalized PSD ')
            legend('1st Singular values','peaks','location','NorthEast')
        end
...
for ii=1:numel(fn)
    ...
    function [Fp] = manualPickPeaking(f,S,Nmodes)
        ...
        %%
        disp('Peak selection procedure')
        disp('a: Draw rectangles around peaks while holding left click')
        disp('b: Press "Space" key to continue the peak selection')
        disp('c: Press "any other key" if you have selected a peak by mistake and want to ignore it')
