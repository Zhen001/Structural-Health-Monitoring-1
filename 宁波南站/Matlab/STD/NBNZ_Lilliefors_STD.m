...
% 备注：与 NBNZ_Lilliefors_RMS.m 只差RMS/STD
...
%% 读取数据
    ...
    % 3sigma法则
    if if_sigma == 1
        STD_mean = mean(STD); STD_std = std(STD);
        STD = STD(abs(STD-STD_mean)<=2*STD_std);
    end
    
%% Lilliefors检验
    figure('visible','off');
    [~,p,~,~] = lillietest(STD,'Alpha',0.05,'MCTol',1e-3);
    data(n,3) = p;
    h = normplot(STD); set(gca,'FontName','Cambria'); set(gcf,'color','w');
