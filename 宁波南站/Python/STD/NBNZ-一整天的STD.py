...
# Function-计算STD
def calculate_STD(lines):
    accelerate = [float(x.split(' ')[-1].strip()) for x in lines]
...
# long = 6000 # 样本长度
# Fs = 100 # 采样频率
# STD_path = main_path + '\\' + 'STD-' + day_specified + '.txt'
...
with open(file_path, 'r') as f:
    while True:
        ...
        if lines[-1] == '':
            break
        STD = calculate_STD(lines)
        prepared_line = transfer_to_prepared_line(start, STD)
        STD_data.append(prepared_line)

# 将结果写入文件
with open(STD_path,'w') as f:
    f.writelines(STD_data)
