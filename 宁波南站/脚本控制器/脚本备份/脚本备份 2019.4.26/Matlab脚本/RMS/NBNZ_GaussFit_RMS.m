...
%% 图形设置
    ...
    xlabel('振动RMS/(m\cdots^{\fontsize{6}-2})','FontName','宋体','FontWeight','bold','FontSize',12,'LineWidth',2,'position',[mean(Xlims) Ylims(1)-0.09*diff(Ylims)])
...
%% 保存图片
    if if_save_pictures
        ...
        picture_path = [picture_main_path,'\\',date_start,' to ',date_end,'  RMS-',number,picture_type];
