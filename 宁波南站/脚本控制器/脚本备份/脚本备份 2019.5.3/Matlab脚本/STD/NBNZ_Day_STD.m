...
%% 绘制STD-GPR时序图
    ...
    % 绘制数据点及预测值
    plot(time_stamp,STD,'.','MarkerSize',5,'Color',[0 0.447 0.741]);
    ...
    ylabel('STD / (m\cdots^{\fontsize{6}-2})','FontName','Cambria Math','FontSize',11.5,'position',[Xlims(1)-0.032*diff(Xlims) mean(Ylims)])
...
%% 保存图片
    if if_save_pictures
        ...
        picture_path = [picture_main_path,'\\','STD-',day_specified];
