...
% 备注：与 NBNZ_GaussFit_STD.m 只差RMS/STD

%% 参数准备
project = 'NBNZ-GaussFit-RMS';
...
%% Step1
%% 循环出某一测点各小时的RMS柱状分布图和高斯拟合曲线
for n = 1:23
    close all
    number = num2str(n,'%02d');
    time_stamp = [number,':00:00.000'];
    RMS_path = ['"',sub_path,'\\',date_start,' to ',date_end,'-',data_type,'-',number,'.txt','"']; % 和Lilliefors生成的数据一样，可共用
...
%% 画频次分布直方图
    ...
    [counts,centers] = hist(RMS, 10); % 计算直方图
...
%% 画概率密度曲线
    ...
    [mu, sigma, muCI, sigmaCI] = normfit(RMS, 0.05); % 分布参数拟合
...
%% 图形设置
    ...
    xlabel('振动RMS/(m\cdots^{\fontsize{6}-2})','FontName','宋体','FontWeight','bold','FontSize',12,'position',[mean(Xlims) Ylims(1)-0.095*diff(Ylims)])
...
%% 保存图片
    if if_save_pictures
        ...
        picture_path = [picture_main_path,'\\',date_start,' to ',date_end,'  RMS-',number];
