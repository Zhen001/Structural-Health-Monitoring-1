...
# long = 6000 # 样本长度
# Fs = 100 # 采样频率
# RMS_path = main_path + '\\' + start_time + ' to ' + end_time + '的所有RMS.txt'
...
for file_path in files_df[start_time: end_time].iloc[:, 0]:
    with open(file_path, 'r') as f:
        while True:
            ...
            if lines[-1]=='':
                print(file_path.split('\\')[-1]+':'+str(len(RMS_Months)))
                break
            for line in lines:
            ...
            RMS_Months.append(prepared_line)

# 将结果写入文件
with open(RMS_path,'w') as f:
    f.writelines(RMS_Months)